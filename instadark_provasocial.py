...
cor_fonte = 'white'
cor_botao = '#4caf50'
cor_botao_acao = '#2196F3'

# Campo para selecionar a pasta de stories
tk.Label(root, text="Pasta com Stories:", bg=cor_fundo, fg=cor_fonte, font=fonte_padrao).pack(pady=5)
entry_pasta_stories = tk.Entry(root, width=50)
entry_pasta_stories.pack(pady=5)
tk.Button(root, text="Selecionar Pasta", command=selecionar_pasta_stories, bg=cor_botao, fg=cor_fonte, font=fonte_padrao).pack(pady=5)

# Campo para selecionar o arquivo de contas
tk.Label(root, text="Arquivo de Contas:", bg=cor_fundo, fg=cor_fonte, font=fonte_padrao).pack(pady=5)
entry_contas = tk.Entry(root, width=50)
entry_contas.pack(pady=5)
tk.Button(root, text="Selecionar Contas", command=selecionar_contas, bg=cor_botao, fg=cor_fonte, font=fonte_padrao).pack(pady=5)

# Botão para postar stories
tk.Button(root, text="Postar Stories", command=postar_stories, bg=cor_botao_acao, fg=cor_fonte, font=fonte_padrao).pack(pady=20)
